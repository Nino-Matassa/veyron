...
class Database {
    ...
    private static boolean populate(Context context, boolean buildFromScratch) { // all records
        ...
        try {
            ...
            while ((nextRecord = csvReader.readNext()) != null) {
                ...
                beanie.total_cases = nextRecord[4].isEmpty() ? 0:Double.parseDouble(nextRecord[4]);
                beanie.new_cases = nextRecord[5].isEmpty() ? 0:Double.parseDouble(nextRecord[5]);
                beanie.new_cases_smoothed = nextRecord[6].isEmpty() ? 0d:Double.parseDouble(nextRecord[6]);
                beanie.total_deaths = nextRecord[7].isEmpty() ? 0:Double.parseDouble(nextRecord[7]);
                beanie.new_deaths = nextRecord[8].isEmpty() ? 0:Double.parseDouble(nextRecord[8]);
    ...
    private boolean addRecordRequest(Beanie beanie) {
        return false;
    }
